function prepareCommands() {
	...
	loadedImageArray = new Array();
	TO_RADIANS = Math.PI/180;
	imageDrawOriginX = 0.5;
	imageDrawOriginY = 0.5;
	imageData = context.createImageData(1,1);
	imageDataPixel = imageData.data;
}
...
var TO_RADIANS;
var imageDrawOriginX;
var imageDrawOriginY;
function DrawImage(name, x, y, rotation) {
	...
	if (img == undefined) {LoadImage(name);} else {
		...
		context.drawImage(img, -(img.width*imageDrawOriginX), -(img.height*imageDrawOriginY));
 
		// and restore the co-ords to how they were when we began
		context.restore(); 
	}
}
function SetImageDrawOrigin(x,y) {
	imageDrawOriginX = x;
	imageDrawOriginY = y;
}
function GetImageHeight(name) {
	var img = loadedImageArray[name];
	if (img != undefined) {
		return img.height;
	}
...
function ImageIsLoaded(name) {
	return (loadedImageArray[name] != undefined);
}
