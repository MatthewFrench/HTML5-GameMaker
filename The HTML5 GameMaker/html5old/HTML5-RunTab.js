...
function compile(debug) {
	...
	"var mouseX = 0;\n"+
	"var mouseY = 0;\n"+
	"$('#gameCanvas').ready(function() {canvas = document.getElementById('gameCanvas');context = canvas.getContext('2d');Game_Open();});\n" + 
	"$('#gameCanvas').keydown(function(e){ lastKeyPressed = e.keyCode; keyPressedArray[lastKeyPressed] = true; Key_Down(); return false; });\n"+
	"$('#gameCanvas').keydown(function(e){ lastKeyPressed = e.keyCode; keyPressedArray[lastKeyPressed] = false; Key_Up(); return false; });\n"+
	"$('#gameCanvas').mousedown(function(e){ mouseX = e.pageX; mouseY = e.pageY; $(this).focus(); Mouse_Down(); return false; });\n"+
	"$('#gameCanvas').mouseup(function(e){ mouseX = e.pageX; mouseY = e.pageY; mousePressed = true; Mouse_Up(); return false; });\n"+
	"$('#gameCanvas').mousemove(function(e){ mouseX = e.pageX; mouseY = e.pageY; mousePressed = false; Mouse_Move(); return false; });\n"+
...
function interpret(string, methodName, className, debug) {
	...
	var searchStart;
	var searchEnd = 0;
	for (var j = 0; j < codeString.length; j++) {
		if (!searching && codeString.substring(j, j+1) == '"') {searchCharacter = codeString.substring(j, j+1); searchStart = j; searching = true;}else
		if (!searching && codeString.substring(j, j+1) == "'") {searchCharacter = codeString.substring(j, j+1); searchStart = j; searching = true;}else
		if (searching) {
			if (codeString.substring(j, j+1) == searchCharacter && codeString.substring(j-1,j) != "\\") {
				searchEnd = j+1;
				stringArray.push(codeString.substring(searchStart,searchEnd));
				//For simplicity sake, replace entire string with a single ' and later replace all of them with the strings
				codeString = codeString.substring(0,searchStart) + stringReplacementCharacter + stringArray.length + stringReplacementCharacter  + codeString.substring(searchEnd,codeString.length);
				j = searchStart;
				searchStart = 0;
				searching = false;
			} else if (j == codeString.length-1) {searching = false;endCompilation = true;}
		}
	}

	if (endCompilation) {return "There was an error regarding strings in the "+methodName+" method of "+className+"."}

	//Loop through the whole code
	//Look for " or '. Without \ in front. Specify the type.
	//Keep looking after until you find the " or ' without a \ in front. Take the whole thing out and replace with a special character + number.
	//If a quote does not finish, produce an error alert and do not complete the compilation.
	...
	for (var j = 0; j < stringArray.length; j++) {
		parsedMethod = parsedMethod.replace(stringReplacementCharacter+(j+1)+stringReplacementCharacter,stringArray[j]);
	}
